...
public class CosAdditionTheorem : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        left = Mathf.Cos(angle1 * Mathf.Deg2Rad) * Mathf.Cos(angle2 * Mathf.Deg2Rad);
        right = Mathf.Sin(angle1 * Mathf.Deg2Rad) * Mathf.Sin(angle2 * Mathf.Deg2Rad);
        if (isMinus)
        {
            additionTheorem = left + right;
            simpleAdditionTheorem = Mathf.Sin(angle2 * Mathf.Deg2Rad);
            directSin = Mathf.Cos((angle1 - angle2) * Mathf.Deg2Rad);
        }
        else
        {
            additionTheorem = left - right;
